...
| 언제 | 무엇을 | 어디서 |
|---|---|---|
| 아침 8시 | 참모가 내 기록에서 나온 **블로그 글감 5개**를 보내옴 → `2번` 처럼 답하면 (경험/원고 덧붙여도 됨) 내 문체로 초안이 와서 다듬어 발행 | 디스코드 |
| 낮 | 데일리노트 기록 (자존노트·컨디션·출력 섹션 포함, 평소 습관 그대로) | 폰 옵시디언 |
| 저녁 20시반 | 참모총장이 먼저 말 걸어옴 → 바로 답장하며 하루 회고/토의 | 디스코드 |
...
## 블로그 파이프라인 — 아침 글감 → 선택 → 내 문체 초안

1. **매일 08:00** 내 최근 기록(데일리노트·자존노트)에서만 뽑은 글감 5개가 디스코드로 옴 (수동: `주제`)
2. `2번` 이라고 답하면 → **볼트의 '블로그백업' 글들을 문체 기준으로** 초안 작성 → 디스코드 + `블로그 초안/` 폴더에 저장
3. 번호 뒤에 내 경험/원고를 덧붙이면 그걸 최우선 재료로 씀. 새 주제는 `글써줘 <주제/원고>`
4. 재료가 부족한 부분은 `[여기에 사장님 경험 추가]`로 비워둠 — 지어내지 않는 것이 원칙
5. 다듬어서 발행 → `완료 블로그 - 제목` 으로 출력 기록
...
**Q. 체크인이 안 왔어요**
→ 20시 반에 PC가 절전이면 깨어나서 보내고, 완전히 꺼져 있었으면 다음에 켤 때 보낸다 (같은 날 중복 발송은 스킵).
